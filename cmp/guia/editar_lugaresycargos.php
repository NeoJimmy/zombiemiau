...
if (isset($_GET['id']) ) {
$idguia_lugaresycargos = (int) $_GET['id'];
...
$sql = "UPDATE `guia_lugaresycargos` SET
`anexo` =  '{$_POST['anexo']}', 
`nombre` =  '{$_POST['nombre']}', 
`unidad` =  '{$_POST['unidad']}', 
`localidad` =  '{$_POST['localidad']}', 
`centro_de_costo` =  '{$_POST['centro_de_costo']}' 
WHERE `idguia_lugaresycargos` = '$idguia_lugaresycargos' ";

//echo $sql."<br />";

mysql_query($sql) or die(mysql_error());
echo (mysql_affected_rows()) ? "Fila Editada.<br />" : "Sin cambios. <br />";
echo "<a href='admin_lugaresycargos.php'>Volver</a><br /><br />";
}

//Llenamos el formulario
$row = mysql_fetch_array ( mysql_query("SELECT * FROM `guia_lugaresycargos` WHERE `idguia_lugaresycargos` = '$idguia_lugaresycargos' "));
?>

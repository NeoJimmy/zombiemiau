...
<h2>Administrar Gu&iacute;a lugares y cargos</h2>
...
// Eliminar elemento
  ...
  {
    $query = 'DELETE FROM guia_lugaresycargos WHERE idguia_lugaresycargos = '.$_GET['delete'];
    mysql_query($query);
    unset($_GET['delete']);
    header('Location:admin_lugaresycargos.php');
  }
?>
<div id="dialog-confirm" title="Eliminar lugar o cargo:">
	<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>&iquest;Desea eliminar este elemento?</p>
</div>
<p>
        <a class="btn btn-success" href="nuevo_lugaresycargos.php">Nuevo registro</a>
</p>
<?php

$sql= "SELECT *
       FROM `guia_lugaresycargos` " ;
...
<?php else : ?>
   ...
   ?>
	...
	<tr>	
            ...
            <ul>
                <li id="accion">
                    <a href="editar_lugaresycargos.php?id=<?php echo $agenda[$i]['idguia_lugaresycargos']; ?>" >Editar</a>
                </li>
                <li id="accion">
                    <a id="opener" href="admin_lugaresycargos.php?delete=<?php echo $agenda[$i]['idguia_lugaresycargos']; ?>"> Eliminar</a>
                </li>
            </ul>
            </div>
            </td>

...
if (isset($_GET['solicitante']) && $_GET['solicitante']!="ninguno") :

	$solicitante = str_replace("%", " ", $_GET['solicitante']);
    ...
    $sql= "SELECT `idorden_de_trabajo`, `nombre`, `apellido`, `anexo`, `ciudad`, `faena`, `area`, `tipo_ot`, `subtipo_ot`, `descripcion`, `observaciones`, `estado`  
	...
	if($rows !=0){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('M7', 'estado');
		...
		for ($i = 0; $i < $rows; $i++):
			...
			$objPHPExcel->getActiveSheet()->setCellValue('M'.$index, $ot[$i]['estado']);
			$index++;
 		endfor;
		...
		header('Content-Disposition: attachment;filename="reporte_solicitante.xlsx"');
